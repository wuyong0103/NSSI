...
library("rcompanion")
#Read the data
data <- read.csv("SuicideIdeation20210807.csv", header = T)
...
formatFit(fit)
HosmerLemeshowTest(fit = fitted(fit), obs = as.factor(data$SelfInjury), X = cbind(as.factor(data$Gender), data$Age, data$Depress, data$Anxiety, data$Stigma, 
                                                                                  ...
                                                                                  data$InterpersonalStress, data$StudyPressure, data$Adaptation, data$Lose, data$Punish))
